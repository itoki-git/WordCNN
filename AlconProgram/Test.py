...
class Test(Train):
    def __init__(self):
        ...
        # 画像ディレクトリ
        self.TestDir = "./Dataset_100Bin/WordPredict_100/"
        self.NameLength = 3 
        self.EOS = "@"
        self.Train = Train()
        Train.__init__(self)
    ...
    def LSTM(self, ImageFile, NameLength):
        ...
        return Output1, Output2, Output3, Image
    ...
    def main(self):        
        ...
        False2 = 0
        False3 = 0
        #File = open("./CV9FalseName.txt", 'a')
        #File.write("    "+ "File名" + "            " + "正解ラベル" + "         " +"不正解ラベル" + "\n")
        self.j = 0
        for ImageFile in Path:
            FileName = os.path.basename(ImageFile)
            WordID, _ = FileName.split('_')
            Name = CreateData().GetID()[WordID]
            Output1, Output2, Output3, Image = self.LSTM(ImageFile, 3)
            ...
            if Name==OutputName:
                ...
                print("False!")
                False2+=1
                #File.write(FileName+ " " + Name + " " + OutputName + "\n")
                #cv2.imwrite("./FalseImage/"+os.path.basename(FileName), Image)
            print(WordID, Name, OutputName)
        print(TrueCount, False2)
#Test().test()